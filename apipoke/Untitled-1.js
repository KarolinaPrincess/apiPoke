...
const Pokemons = (() => {
    ...
    // Función para obtener el número del Pokémon de la URL
    function obtenerNumeroPokemon() {
        const url = document.location.href;
        const numeroPokemonStr = obtenerParametroDeURL(url, 'numero');
        
        if (numeroPokemonStr) {
            // Convierte el número de la URL a un entero y almacénalo en la variable global
            numeroPokemon = parseInt(numeroPokemonStr, 10);

            if (!isNaN(numeroPokemon)) {
                // Llama a las funciones para mostrar los detalles y estadísticas del Pokémon
                _mostrarDetallesPokemons(numeroPokemon);
                _mostrarStats(numeroPokemon);
            } else {
                console.error('Número de Pokémon no válido en la URL.');
            }
        } else {
            console.error('Número del Pokémon no encontrado en la URL.');
        }
    }
